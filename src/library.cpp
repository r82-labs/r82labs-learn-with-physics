...
namespace {
    ...
    Simulator::Simulator(const SimulatorConfig& config)
        : gravity(config.g), projectile(config.projectile), x_velocity_factor(0.0f), y_velocity_factor(0.0f), half_gravity_factor(0.5f * config.g) {
        ...
        const float v0 = config.draw_length * std::sqrt((config.slingshot.get_efficiency() * config.slingshot.get_stiffness()) / projectile.get_mass());
    ...
    Point Simulator::get_position_at_time(const TimeRequest& request) const {
        return {
            x_velocity_factor * request.time,
